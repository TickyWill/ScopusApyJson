...
def build_scopus_df_from_api(doi_list, timeout = None, verbose = False):
    ...
    Note:
        The function `get_doi_json_data_from_api` is imported from the 'api_manager' module.
        The function `parse_json_data_to_scopus_df` is imported from the 'json_parser' module.
    ...
    for _, doi in enumerate(doi_list):        
        ...
        if request_status == "False":
            authy_status = False
            if verbose : print('Authentication failed: please check availability of authentication keys')
            break
        else:
            authy_status = True
            if request_status == "Empty":
                if verbose : print(f'DOI {doi} not found in Scopus database')
                failed_doi_df.loc[failed_num, "DOI"] = doi
                failed_doi_df.loc[failed_num, "Fail reason"] = "Not found"
                failed_num += 1            
            elif request_status == "Timeout":
                if verbose : print('Request timeout: please check web access')
                failed_doi_df.loc[failed_num, "DOI"] = doi
                failed_doi_df.loc[failed_num, "Fail reason"] = "Timeout"
                failed_num += 1            
            else:
                if verbose : print(f'Request successful for DOI {doi}')           
